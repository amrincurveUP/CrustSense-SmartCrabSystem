...
static void publish_readings(void)
{
    ...
    int msg_id = esp_mqtt_client_publish(
        s_client, MQTT_TOPIC_READINGS, json, (int)len, MQTT_QOS, MQTT_RETAIN_READINGS);
    if (msg_id >= 0) {
        ESP_LOGI(TAG, "Published %u bytes → %s", (unsigned)len, MQTT_TOPIC_READINGS);
    }
}
...
esp_err_t aq_mqtt_start(void)
{
    ...
    ESP_LOGI(TAG, "Publishing to %s every %d ms", MQTT_TOPIC_READINGS, MQTT_PUBLISH_INTERVAL_MS);
    return ESP_OK;
}
